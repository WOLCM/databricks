...
# MAGIC
# MAGIC Meanwhile, we're inserting the pricing data with an helper. **Please consider this table and the following queries as educational only, using list price, not contractual. Please review your contract for more accurate information.**

# COMMAND ----------

# DBTITLE 1,Pricing tables
## Create the pricing lookup tables
forecast_helper = UsageForecastHelper(spark)
print(f"creating princing table under catalog={catalog}, schema={schema}")
forecast_helper.create_pricing_tables(catalog, schema)
...
# MAGIC
# MAGIC SKUs are detailed per lign of product and per region. To simplify out billing dashboard, we'll merge them under common SKU types i.e. grouping `STANDARD_ALL_PURPOSE_COMPUTE` and `PREMIUM_ALL_PURPOSE_COMPUTE` as `ALL_PURPOSE`

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from system.billing.usage 
# MAGIC   left join sku_cost_lookup on sku_name=sku limit 10

# COMMAND ----------

# DBTITLE 1,Group by main SKU category
from pyspark.sql.functions import col, when, sum, current_date, lit
data_to_predict = spark.sql("""select workspace_id, usage_date as ds, sku_name as sku, cast(usage_quantity as double) as dbus, cast(list_price*usage_quantity as double) as cost_at_list_price 
...
# MAGIC
# MAGIC Once aggregated at a daily level, billing information can be forecasted as a timeserie. For this demo, we will use a simple Prophet model to extend the timeseries to the next quarter 
...
# MAGIC [Open the dashboard](/sql/dashboards/e23b9972-5574-4fbc-8ae7-b4e6c5ac7721) to start exploring your billing data.<br/>
